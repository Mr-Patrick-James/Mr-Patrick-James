...
function files(index) {
  return `./assets/images/bg_male/male${String(index + 1).padStart(4, "0")}.png`;
}
...
// 4. OPTIONAL PAGE PINNING
["#page2", "#page3"].forEach((selector) => {
  gsap.to(selector, {
    scrollTrigger: {
      trigger: selector,
      start: "top top",
...
// 5. SCALE FLOAT — scoped only to the scroll-animation pages
gsap.to("#character-canvas", {
  scale: 1.1,
  y: -90,
  ease: "none",
  scrollTrigger: {
...
// 6. DYNAMIC HEIGHT CALCULATION (shortened)
window.addEventListener("load", () => {
  const scrollEndSpacer = document.getElementById("scroll-end-spacer");
  const sectionCount = document.querySelectorAll(".page").length;
  const footerCount = document.querySelectorAll("footer").length;
  const sectionHeight = window.innerHeight;
  const pinDuration = 16000;

  const totalNeededHeight =
    pinDuration + sectionCount * sectionHeight + footerCount * 800;

  document.querySelector("#main").style.minHeight = `${totalNeededHeight}px`;

  if (scrollEndSpacer) {
    scrollEndSpacer.style.height = "0px";
  }
});
